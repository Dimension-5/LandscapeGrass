...
void FLiteGPUSceneVertexFactoryShaderParameters::Bind(const FShaderParameterMap& ParameterMap)
{
	...
	VertexFetch_PerInstanceScale.Bind(ParameterMap, TEXT("VertexFetch_PerInstanceScale"));
...
) const
{
	...
	FLiteGPUSceneProxy* SceneProxy = BatchElementParams->SceneProxy;
	if (SceneProxy)
	{
		FLiteGPUSceneInstanceDataPtr SharedPerInstanceData = SceneProxy->SharedPerInstanceData;
		ShaderBindings.Add(VertexFetch_PerInstanceTransformParameter, SharedPerInstanceData->RWInstanceTransformBuffer->SRV);
		ShaderBindings.Add(VertexFetch_PerInstanceScale, SharedPerInstanceData->RWInstanceScaleBuffer->SRV);
	}
}
